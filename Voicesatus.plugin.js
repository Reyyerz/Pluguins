//META{"name":"Voicestatus"}*//
class Voicestatus {
	getName () {return "Voicestatus";}
	...
	constructor () {
		this.delay=100;
		this.serverId='464116508252045312';
		...
		this.userContextMenuMarkup = {
			...
			`<span class="Voicestatus-modal DevilBro-modal">
	...
	start () {
		if (typeof BDfunctionsClays !== "object" || BDfunctionsClays.isLibraryOutdated()) {
			if (typeof BDfunctionsClays === "object") BDfunctionsClays = "";
			$('head script[src="https://raw.githubusercontent.com/darten73/BetterPlugins/master/BDfunctionsClays.js"]').remove();
			$('head').append('<script src="https://raw.githubusercontent.com/darten73/BetterPlugins/master/BDfunctionsClays.js"></script>');
		...
		if (typeof BDfunctionsClays === "object") {
			BDfunctionsClays.loadMessage(this);
            this.MemberPerms = BDfunctionsClays.WebModules.findByProperties(["getNicknames", "getNick"]);
			this.ChannelStore = BDfunctionsClays.WebModules.findByProperties(['getChannels']);
			this.CurrentChannel = BDfunctionsClays.WebModules.findByProperties(['getChannelId']);
			this.CurrentUser = BDfunctionsClays.WebModules.findByProperties(['getCurrentUser']);
			this.ChannelActions = BDfunctionsClays.WebModules.findByProperties(['selectVoiceChannel']);
			this.UsersVoiceStore= BDfunctionsClays.WebModules.findByProperties(['getVoiceStates']);
			this.documentObserver.observe(document.querySelector('#app-mount'), {childList: true, subtree: true});
			this.adminActions=BDfunctionsClays.WebModules.findByProperties(['move']);
			this.GuildStore = BDfunctionsClays.WebModules.findByProperties(["getGuilds"]);
	...
	stop () {
		if (typeof BDfunctionsClays === "object") {
			...
			BDfunctionsClays.removeLocalStyle(this.getName());
			BDfunctionsClays.unloadMessage(this);
		}
	}
	...
	onPopouts(node){
        let react = BDfunctionsClays.getReactInstance(node).child.memoizedProps;
        	let info=react.user;
	...
	find(info, destroy){
		...
		let s;
        BDfunctionsClays.showToast((s=this.ChannelStore.getChannel(userChannelId))?'Channel: '+s:'не найден');
					...
					if(tmpPerm=s.permissionOverwrites[id]){
						...
						if(deny >= 1048576){
							...
							if(tmp[tmp.length-21]==='1'&&canConnect!==true) {
								BDfunctionsClays.showToast("Нет доступа на вход в канал");
								canConnect=false;
	...
	onContextMenu (context) {
        let serverObj = BDfunctionsClays.getSelectedServer();
        serverObj=serverObj?serverObj: BDfunctionsClays.getSelectedChannel();
        ...
        let info = BDfunctionsClays.getKeyInformation({"node":context, "key":"user"});
		if (info && BDfunctionsClays.getKeyInformation({"node":context, "key":"displayName", "value":"UserNoteItem"})) {
			...
			for(let group in this.userContextMenuMarkup){
                ...
                for(let item in this.userContextMenuMarkup[group]){
					if(BDfunctionsClays.getData(item,this,"settings")) {
			...
			$(context).append(userContextMenuMarkup)
				.on("click", ".moveToAfk-item", ()=>{
					$(context).hide();
					this.adminActions.setChannel(this.serverId,info.id,'464116508252045312');
				})
				.on("click", ".find-item", () => {
					$(context).hide();
					this.find(info);
	...
	showTribunalSettings(info, type){
		...
		BDfunctionsClays.appendModal(userTribunalSettings);
	...
	getSettingsPanel() {
		if (!this.started || typeof BDfunctionsClays !== "object") return;
        let settings = BDfunctionsClays.getAllData(this, "settings");
		...
		BDfunctionsClays.initElements(settingspanel);
		$(settingspanel)
	...
	updateSettings(settingspanel) {
		...
		BDfunctionsClays.saveAllData(settings, this, "settings");
	  }

	findChannel(info){
        this.isFind=false;
        let found=this.usersInVoice.get(info.id);
        if(found) BDfunctionsClays.showToast('Channel: '+this.ChannelStore.getChannel(found.id));
	...
	sendMessage(textarea, commadn, userId, description){
		const prefix=BDfunctionsClays.getData("debag", this, "settings")?"?":"!";
