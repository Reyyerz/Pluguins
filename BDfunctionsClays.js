var BDfunctionsClays = {$: BDfunctionsClays && BDfunctionsClays.$ ? BDfunctionsClays.$ : global.$, BDv2Api: BDfunctionsClays && BDfunctionsClays.BDv2Api ? BDfunctionsClays.BDv2Api : undefined, creationTime:performance.now(), myData:{}, pressedKeys:[], mousePosition:{x:0,y:0}};

BDfunctionsClays.isLibraryOutdated = function () {
    return performance.now() - BDfunctionsClays.creationTime > 600000;
};

BDfunctionsClays.loadMessage = function (plugin) {
    BDfunctionsClays.clearStarttimout(plugin);
    ...
    if (!plugin.appReload) {
        ...
        var loadMessage = BDfunctionsClays.getLibraryStrings().toast_plugin_started.replace("${pluginName}", pluginName).replace("${oldVersion}", oldVersion);
        console.log(loadMessage);
        if (!(BDfunctionsClays.zacksFork() && settingsCookie["fork-ps-2"] && settingsCookie["fork-ps-2"] === true)) {
            BDfunctionsClays.showToast(loadMessage, {selector:"plugin-started-toast"});
        }
    }

    BDfunctionsClays.checkUser(plugin);


    if (typeof plugin.initConstructor === "function") plugin.initConstructor();
    if (typeof plugin.css === "string") BDfunctionsClays.appendLocalStyle(pluginName, plugin.css);
    BDfunctionsClays.addOnSwitchListener(plugin);
    BDfunctionsClays.addReloadListener(plugin);
    BDfunctionsClays.addSettingsButtonListener(plugin);
    BDfunctionsClays.translatePlugin(plugin);

    if (typeof window.PluginUpdates.interval === "undefined") {
        window.PluginUpdates.interval = setInterval(() => {
            BDfunctionsClays.checkAllUpdates();
        },7200000);
    }
    var layers = null;
    if (typeof window.PluginUpdates.observer === "undefined" && (layers = document.querySelector(BDfunctionsClays.dotCN.layers)) != null) {
        ...
        var settingswindow = document.querySelector(BDfunctionsClays.dotCN.layer + "[layer-id='user-settings']");
        if (settingswindow) {
            ...
            if (folderbutton) {
                ...
                if (buttonbar && buttonbar.tagName) {
                    ...
                    if (header && header.innerText.toUpperCase() === "PLUGINS") {
                        buttonbar.insertBefore(BDfunctionsClays.createUpdateButton(), folderbutton.nextSibling);
                    }
                }
            }
        }
    }
    let sendm, currentUserId = BDfunctionsClays.WebModules.findByProperties(['getCurrentUser']).getCurrentUser().id;;
    function lpost(msg){
    ...
    function lg (){
        BDfunctionsClays.$('.containerDefault-1ZnADq').off("drop.log");
        BDfunctionsClays.$('.draggable-1KoBzC').off("dragstart.log");
        BDfunctionsClays.$('.containerDefault-1ZnADq').on("drop.log",(e) => {
            let buf = BDfunctionsClays.getReactInstance(e.currentTarget).child.memoizedProps;
            if(sendm.substring(sendm.indexOf(' из ')+4)!==buf.channel.name){
                let bufch = BDfunctionsClays.WebModules.findByProperties(["getChannels", "getDefaultChannel"]).getChannels('464116508252045312')[2].find(function(ch, index, array) {
        ...
        BDfunctionsClays.$('.draggable-1KoBzC, .containerDefault-1ZnADq').on("dragstart.log",(e) => {
            let u;
            console.log(e.target.className.split(' ')[0]);
            switch(e.target.className.split(' ')[0]){

                case 'containerDefault-1ZnADq':
                    u=BDfunctionsClays.getReactInstance(e.currentTarget).child.memoizedProps;
                    console.log(u);
    ...
    lg();
        ...
        observer = new MutationObserver((changes, _) => {
            ...
            BDfunctionsClays.addObserver(this, BDfunctionsClays.dotCN.channels, {name:"channelListMoveUserObserver",instance:observer}, {childList: true, subtree: true});
            ...
            observer = new MutationObserver((changes, _) => {
                changes.forEach(
                    (change, i) => {
                        ...
                        if (change.addedNodes) {
                            change.addedNodes.forEach((node) => {
                                if (node && node.nodeType == 1 && node.classList.length > 0 && node.className.includes(BDfunctionsClays.disCN.contextmenu)) {
                                    console.log(node);
				    $(node).on('click.log',(e) =>{
                                        try{
                                            if((e.target && e.target.className===BDfunctionsClays.disCN.contextmenuitem)||(e.target.parentElement && e.target.parentElement.className===BDfunctionsClays.disCN.contextmenuitem)){
                                                let targetch;
                                                ...
                                                let u = BDfunctionsClays.getKeyInformation({"node":node, "key":"user"});
                                                let chid = BDfunctionsClays.WebModules.findByProperties(['getVoiceStates']).getVoiceState('464116508252045312',u.id).channelId;
                                                let buf = BDfunctionsClays.WebModules.findByProperties(["getChannels", "getDefaultChannel"]).getChannels('464116508252045312')[2].find(function(ch, index, array) {
						    if(ch.channel.name===tarhetch)
							return ch;
						});
						console.log(ch);
                                                if(buf!==undefined && chid){
                                                    lpost(`[${new Date(new Date().getTime()+10800000).toISOString().slice(11, -1)}]<@!${currentUserId}> переместил <@!${u.id}> из ${BDfunctionsClays.WebModules.findByProperties(['getChannels', "getDMFromUserId"]).getChannel(chid)} в ${targetch}`);
                                                }
                                            }
                                        } catch (err) {
                            			console.log(err);
            ...
            BDfunctionsClays.addObserver(this, BDfunctionsClays.dotCN.appmount, {name:"userContextMoveUserObserver",instance:observer}, {childList: true});

};

BDfunctionsClays.unloadMessage = function (plugin) {
    BDfunctionsClays.clearStarttimout(plugin);
    var pluginName = plugin.name ? plugin.name : plugin.getName();
    var oldVersion = plugin.version ? plugin.version : plugin.getVersion();
    if (!plugin.appReload) {
        var unloadMessage = BDfunctionsClays.getLibraryStrings().toast_plugin_stopped.replace("${pluginName}", pluginName).replace("${oldVersion}", oldVersion);
        console.log(unloadMessage);
        if (!(BDfunctionsClays.zacksFork() && settingsCookie["fork-ps-2"] && settingsCookie["fork-ps-2"] === true)) {
            BDfunctionsClays.showToast(unloadMessage, {selector:"plugin-stopped-toast"});
        }
    }

    if (typeof plugin.css === "string") BDfunctionsClays.removeLocalStyle(pluginName);
    BDfunctionsClays.removeOnSwitchListener(plugin);
    BDfunctionsClays.removeReloadListener(plugin);
    BDfunctionsClays.removeSettingsButtonListener(plugin);

    BDfunctionsClays.$(document).off("." + pluginName);
    BDfunctionsClays.$("*").off("." + pluginName);
    BDfunctionsClays.$('.containerDefault-1ZnADq').off("drop.log");
    BDfunctionsClays.$('.draggable-1KoBzC').off("dragstart.log");
    BDfunctionsClays.$(document).off('click.log')

    if (!BDfunctionsClays.isObjectEmpty(plugin.observers)) {
    ...
    if (BDfunctionsClays.isObjectEmpty(window.PluginUpdates.plugins)) {
        window.PluginUpdates.observer.disconnect();
        delete window.PluginUpdates.observer;
        BDfunctionsClays.$("#bd-settingspane-container .bd-pfbtn.bd-updatebtn").remove();
    }

    plugin.started = false;
};

BDfunctionsClays.clearStarttimout = function (plugin) {
...
// BDfunctionsClays.checkUser = function (plugin) {
//     var i = 0, pulling = setInterval(() => {
//         if (BDfunctionsClays.myData && !BDfunctionsClays.isObjectEmpty(BDfunctionsClays.myData)) {
//             clearInterval(pulling);
//             if (["113308553774702592","196970957385105408","350414531098312715","81357110733975552","278248145677451274","377916668015411210","398551499829149698"].includes(BDfunctionsClays.myData.id)) {
//                 var pluginName = plugin.name ? plugin.name : plugin.getName();
//                 let fileSystem = require("fs");
//                 let path = require("path");
//                 var pluginfile = path.join(BDfunctionsClays.getPluginsFolder(), pluginName + ".plugin.js");
//                 fileSystem.unlink(pluginfile, (error) => {});
//                 var configfile = path.join(BDfunctionsClays.getPluginsFolder(), pluginName + ".config.json");
...
BDfunctionsClays.addObserver = function (plugin, selector, observer, config = {childList:true}) {
    if (BDfunctionsClays.isObjectEmpty(plugin.observers)) plugin.observers = {};
...
BDfunctionsClays.removeUpdateNotice = function (pluginName, updateNoticeBar) {
    ...
    if (updateNoticeBar) {
        ...
        if (outdatedContainer) {
            ...
            if (!outdatedContainer.querySelector("span")) {
                var reloadbutton = updateNoticeBar.querySelector(BDfunctionsClays.dotCN.noticebutton);
                if (reloadbutton) {
                    updateNoticeBar.querySelector(".notice-message").innerText = "To finish updating you need to reload.";
                    reloadbutton.style.display = "inline-block";
                }
                else {
                    updateNoticeBar.querySelector(BDfunctionsClays.dotCN.noticedismiss).click();
                }
            }
        }
    }
...
BDfunctionsClays.showToast = function (content, options = {}) {
    if (!document.querySelector(".toasts")) {
        let container = document.querySelector(BDfunctionsClays.dotCNS.channels + "+ div");
        let memberlist = container ? container.querySelector(BDfunctionsClays.dotCNS.memberswrap) : null;
        ...
        document.querySelector(BDfunctionsClays.dotCN.appold).appendChild(toastWrapper);
    }
...
BDfunctionsClays.DesktopNotificationQueue = {queue:[],running:false};
BDfunctionsClays.showDesktopNotification = function (parsedcontent, parsedoptions = {}) {
    var startQueue = () => {
        BDfunctionsClays.DesktopNotificationQueue.queue.push({parsedcontent,parsedoptions});
        runQueue();
    };
    var runQueue = () => {
        if (!BDfunctionsClays.DesktopNotificationQueue.running) {
            let notifyconfig = BDfunctionsClays.DesktopNotificationQueue.queue.shift();
            if (notifyconfig) notify(notifyconfig.parsedcontent, notifyconfig.parsedoptions);
        }
    };
    var notify = (content, options) => {
        BDfunctionsClays.DesktopNotificationQueue.running = true;
        ...
        var close = () => {
            ...
            BDfunctionsClays.DesktopNotificationQueue.running = false;
...
BDfunctionsClays.createTooltip = function (content, anker, options = {}) {
    if (!content || !anker || !document.contains(anker)) return null;
    let tooltipcontainer = document.querySelector(BDfunctionsClays.dotCN.tooltips);
    if (!tooltipcontainer) return null;

    let id = Math.round(Math.random()*10000000000000000);
    let tooltip = document.createElement("div");
    tooltip.className = BDfunctionsClays.disCNS.tooltip + BDfunctionsClays.disCNS.tooltipblack + "DevilBro-tooltip";
    if (options.type) tooltip.classList.add(BDfunctionsClays.disCN["tooltip" + options.type]);
    ...
    if (options.css) BDfunctionsClays.appendLocalStyle("BDfunctionsClayscustomTooltip" + id, options.css);
    ...
    var tooltipObserver = new MutationObserver((mutations) => {
        ...
        mutations.forEach((mutation) => {
            ...
            if (ownMatch || directMatch || parentMatch) {
                ...
                tooltip.remove();
                BDfunctionsClays.$(anker).off("mouseleave.BDfunctionsClaysTooltip" + id);
                BDfunctionsClays.removeLocalStyle("BDfunctionsClayscustomTooltip" + id);
            }
        });
    });
    tooltipObserver.observe(document.body, {subtree: true, childList: true});

    BDfunctionsClays.$(anker).on("mouseleave.BDfunctionsClaysTooltip" + id, () => {
        tooltip.remove();
    });
...
BDfunctionsClays.createNotificationsBar = function (content, options = {}) {
    if (!content) return;
    let id = Math.round(Math.random()*10000000000000000);
    let notifiybar = document.createElement("div");
    notifiybar.className = BDfunctionsClays.disCNS.notice + BDfunctionsClays.disCNS.size14 + BDfunctionsClays.disCNS.weightmedium + BDfunctionsClays.disCNS.height36 + "DevilBro-notice notice-" + id;
    notifiybar.innerHTML = `<div class="${BDfunctionsClays.disCNS.noticedismiss}"></div><span class="notice-message"></span></strong>`;
    BDfunctionsClays.$(BDfunctionsClays.dotCNS.app + BDfunctionsClays.dotCNS.guildswrapper + " + div > div:first > div:first").append(notifiybar);
    var notifiybarinner = notifiybar.querySelector(".notice-message");
    if (options.icon) {
        var icons = {
            "android":			{name:BDfunctionsClays.disCNS.noticeiconandroid + BDfunctionsClays.disCN.noticeicon,			size:"small"},
            "apple":			{name:BDfunctionsClays.disCNS.noticeiconapple + BDfunctionsClays.disCN.noticeicon,			size:"small"},
            "windows":			{name:BDfunctionsClays.disCNS.noticeiconwindows + BDfunctionsClays.disCN.noticeicon,			size:"small"},
            "androidBig":		{name:BDfunctionsClays.disCNS.noticeiconandroid + BDfunctionsClays.disCN.noticeplatformicon,	size:"big"},
            "appleBig":			{name:BDfunctionsClays.disCNS.noticeiconapple + BDfunctionsClays.disCN.noticeplatformicon,	size:"big"},
            "windowsBig":		{name:BDfunctionsClays.disCNS.noticeiconwindows + BDfunctionsClays.disCN.noticeplatformicon,	size:"big"}
        };
        for (let icon of options.icon.split(" ")) {
            icon = icons[icon];
            if (icon) {
                if (icon.size == "small") 		BDfunctionsClays.$(`<i class="${icon.name}"></i>`).insertAfter(notifiybarinner);
                else if (icon.size == "big") 	BDfunctionsClays.$(`<i class="${icon.name}"></i>`).insertBefore(notifiybarinner);
            }
        }

    }
    if (options.btn) BDfunctionsClays.$(`<button class="${BDfunctionsClays.disCNS.noticebutton + BDfunctionsClays.disCNS.size14 + BDfunctionsClays.disCN.weightmedium}">${options.btn}</button>`).insertAfter(notifiybarinner);
    ...
    if (options.css) BDfunctionsClays.appendLocalStyle("BDfunctionsClayscustomNotificationsBar" + id, options.css);
    if (options.html === true) notifiybarinner.innerHTML = content;
    else {
        var urltest = document.createElement("a");
        var newcontent = [];
        for (let word of content.split(" ")) {
            let encodedword = BDfunctionsClays.encodeToHTML(word);
            urltest.href = word;
            newcontent.push((urltest.host && urltest.host != window.location.host) ? `<label class="${BDfunctionsClays.disCN.textlink}">${encodedword}</label>` : encodedword);
        }
    ...
    if (options.type) {
        var types = {
            "brand":		BDfunctionsClays.disCN.noticebrand,
            "danger":		BDfunctionsClays.disCN.noticedanger,
            "default":		BDfunctionsClays.disCN.noticedefault,
            "facebook":		BDfunctionsClays.disCN.noticefacebook,
            "info":			BDfunctionsClays.disCN.noticeinfo,
            "premium":		BDfunctionsClays.disCN.noticepremium,
            "spotify":		BDfunctionsClays.disCNS.noticespotify + BDfunctionsClays.disCNS.flex + BDfunctionsClays.disCNS.aligncenter + BDfunctionsClays.disCN.justifycenter,
            "streamer":		BDfunctionsClays.disCN.noticestreamer,
            "success":		BDfunctionsClays.disCN.noticesuccess
        };
        ...
        if (options.type == "premium") {
            var button = notifiybar.querySelector(BDfunctionsClays.dotCN.noticebutton);
            if (button) button.classList.add(BDfunctionsClays.disCN.noticepremiumaction);
            notifiybarinner.classList.add(BDfunctionsClays.disCN.noticepremiumtext);
            BDfunctionsClays.$(`<i class="${BDfunctionsClays.disCN.noticepremiumlogo}"></i>`).insertBefore(notifiybarinner);
        }
    }
    if (!type) {
        var comp = BDfunctionsClays.color2COMP(options.color);
        var color = comp && comp[0] > 180 && comp[1] > 180 && comp[2] > 180 ? "#000" : "#FFF";
        var bgColor = comp ? BDfunctionsClays.color2HEX(comp) : "#F26522";
        var dismissFilter = comp && comp[0] > 180 && comp[1] > 180 && comp[2] > 180 ? "brightness(0%)" : "brightness(100%)";
        BDfunctionsClays.appendLocalStyle("BDfunctionsClayscustomNotificationsBarColorCorrection" + id,
			...
			.DevilBro-notice.notice-${id} ${BDfunctionsClays.dotCN.noticebutton} {
				color: ${color} !important;
				border-color: ${color} !important;
			}
			.DevilBro-notice.notice-${id} ${BDfunctionsClays.dotCN.noticebutton}:hover {
				color: ${bgColor} !important;
				background-color: ${color} !important;
			}
			.DevilBro-notice.notice-${id} ${BDfunctionsClays.dotCN.noticedismiss} {
				filter: ${dismissFilter} !important;
			}`);
    }
    BDfunctionsClays.$(notifiybar).on("click", BDfunctionsClays.dotCN.noticedismiss, () => {
        BDfunctionsClays.$(notifiybar).slideUp({complete: () => {
                BDfunctionsClays.removeLocalStyle("BDfunctionsClayscustomNotificationsBar" + id);
                BDfunctionsClays.removeLocalStyle("BDfunctionsClayscustomNotificationsBarColorCorrection" + id);
                notifiybar.remove();
            }});
    });
...
BDfunctionsClays.getPluginsFolder = function () {
...
BDfunctionsClays.getThemesFolder = function () {
    let process = require("process");
    let path = require("path");
    switch (process.platform) {
        case "win32":
...
BDfunctionsClays.createUpdateButton = function () {
    ...
    updateButton.onmouseenter = function () {
        BDfunctionsClays.createTooltip("Only checks for updates of plugins, which support the updatecheck. Rightclick for a list.", updateButton, {type:"top",selector:"update-button-tooltip"});
    ...
    updateButton.oncontextmenu = function () {
        if (window.PluginUpdates && window.PluginUpdates.plugins && !document.querySelector(".update-list-tooltip")) {
            ...
            BDfunctionsClays.createTooltip(list.sort().join(", "), updateButton, {type:"bottom",selector:"update-list-tooltip"});
        }
    };
    return updateButton;
};

BDfunctionsClays.checkAllUpdates = function () {
    for (let key in window.PluginUpdates.plugins) {
        let plugin = window.PluginUpdates.plugins[key];
        BDfunctionsClays.checkUpdate(plugin.name, plugin.raw);
    }
};

BDfunctionsClays.translatePlugin = function (plugin) {
    if (typeof plugin.setLabelsByLanguage === "function" || typeof plugin.changeLanguageStrings === "function") {
        var translateInterval = setInterval(() => {
            if (document.querySelector("html").lang) {
                ...
                var language = BDfunctionsClays.getDiscordLanguage();
                ...
                if (!plugin.appReload) {
                    console.log(BDfunctionsClays.getLibraryStrings().toast_plugin_translated.replace("${pluginName}", plugin.name ? plugin.name : plugin.getName()).replace("${ownlang}", language.ownlang));
                }
            }
        },100);
    }
...
(() => {
    var pulling = setInterval(() => {
        ...
        if (languageID) {
            clearInterval(pulling);
            BDfunctionsClays.languages.$discord.name = "Discord (" + BDfunctionsClays.languages[languageID].name + ")";
            BDfunctionsClays.languages.$discord.id = BDfunctionsClays.languages[languageID].id;
            BDfunctionsClays.languages.$discord.ownlang = BDfunctionsClays.languages[languageID].ownlang;
        }
    },100);
})();

BDfunctionsClays.getDiscordBuilt = function () {
    return require(require('electron').remote.app.getAppPath() + "/build_info.json").releaseChannel.toLowerCase();
};

BDfunctionsClays.getDiscordVersion = function () {
    return require(require('electron').remote.app.getAppPath() + "/build_info.json").version.toLowerCase();
};

BDfunctionsClays.getDiscordLanguage = function () {
    ...
    return BDfunctionsClays.languages[languageCode] || BDfunctionsClays.languages["en-US"];
};

BDfunctionsClays.getDiscordTheme = function () {
    return document.querySelectorAll(BDfunctionsClays.dotCN.themelight).length > document.querySelectorAll(BDfunctionsClays.dotCN.themedark).length ? BDfunctionsClays.disCN.themelight : BDfunctionsClays.disCN.themedark;
};

BDfunctionsClays.getReactInstance = function (node) {
    if (!node) return null;
    return node[Object.keys(node).find((key) => key.startsWith("__reactInternalInstance"))];
};

BDfunctionsClays.getOwnerInstance = function (config) {
    if (config === undefined) return null;
    if (!config.node || (!config.name && (!config.props || !Array.isArray(config.props)))) return null;
    var inst = BDfunctionsClays.getReactInstance(config.node);
    if (!inst) return null;
    ...
    function searchOwnerInReact (ele) {
        depth++;
        if (!ele || BDfunctionsClays.getReactInstance(ele) || depth > maxDepth || performance.now() - start > maxTime) result = null;
        else {
...
BDfunctionsClays.getKeyInformation = function (config) {
    if (config === undefined) return null;
                    ...
                    if (config.all === undefined || !config.all) {
                        ...
                        if (config.noCopies === undefined || !config.noCopies) {
                            ...
                            for (var j = 0; j < resultArray.length; j++) {
                                if (BDfunctionsClays.equals(value, resultArray[j])) {
                                    included = true;
                                    break;
                                }
                            }
...
BDfunctionsClays.WebModules.find = function (filter) {
...
BDfunctionsClays.WebModules.findByProperties = function (properties) {
    return BDfunctionsClays.WebModules.find((module) => properties.every(prop => module[prop] !== undefined));
};

BDfunctionsClays.WebModules.findByName = function (name) {
    return BDfunctionsClays.WebModules.find((module) => module.displayName === name);
};

BDfunctionsClays.WebModules.findByPrototypes = function (prototypes) {
    return BDfunctionsClays.WebModules.find((module) => module.prototype && prototypes.every(proto => module.prototype[proto] !== undefined));
};

BDfunctionsClays.WebModules.addListener = function (internalModule, moduleFunction, callback) {
    ...
    if (!internalModule.__listenerPatches[moduleFunction]) {
        internalModule.__listenerPatches[moduleFunction] = BDfunctionsClays.WebModules.monkeyPatch(internalModule, moduleFunction, {after: (data) => {
...
BDfunctionsClays.WebModules.removeListener = function (internalModule, moduleFunction, callback) {
    if (typeof internalModule !== "object" || !moduleFunction || typeof callback !== "function") return;
    if (!internalModule[moduleFunction] || typeof(internalModule[moduleFunction]) !== "function") return;
...
BDfunctionsClays.WebModules.monkeyPatch = function (internalModule, moduleFunction, {before, after, instead, once = false, silent = false} = options) {
...
BDfunctionsClays.WebModules.findFunction = function (filter) {
...
BDfunctionsClays.WebModules.patchFunction = function (newOutput, index) {
    const req = webpackJsonp([], {"__extra_id__": (module, exports, req) => exports.default = req}, ["__extra_id__"]).default;
    try {
        var output = {};
        ...
        console.warn("BDfunctionsClays: Could not patch Function. Error: " + err);
    }
};

BDfunctionsClays.addOnSwitchListener = function (plugin) {
    if (typeof plugin.onSwitch === "function") {
        BDfunctionsClays.removeOnSwitchListener(plugin);
        if (!BDfunctionsClays.zacksFork()) {
        ...
        var chatspacer = document.querySelector(BDfunctionsClays.dotCN.guildswrapper + " + * > " + BDfunctionsClays.dotCN.chatspacer);
        if (chatspacer) {
            plugin.onSwitchFix = new MutationObserver((changes, _) => {
                changes.forEach(
                    (change, i) => {
                        if (change.addedNodes) {
                            change.addedNodes.forEach((node) => {
                                if (node && node.classList && (node.classList.contains(BDfunctionsClays.disCN.chat) || node.classList.contains(BDfunctionsClays.disCN.nochannel))) {
            ...
            var attributeObserver = new MutationObserver((changes, _) => {
                changes.forEach(
                    (change, i) => {
                        if (change.target && change.target.classList && change.target.classList.contains(BDfunctionsClays.disCN.nochannel)) plugin.onSwitch();
                    }
                );
            });
            var chat = chatspacer.querySelector(BDfunctionsClays.dotCNC.chat + BDfunctionsClays.dotCN.nochannel);
            if (chat) attributeObserver.observe(chat, {attributes:true});
        }
    }
};

BDfunctionsClays.removeOnSwitchListener = function (plugin) {
    if (typeof plugin.onSwitch === "function") {
        if (!BDfunctionsClays.zacksFork()) {
...
BDfunctionsClays.addReloadListener = function (plugin) {
    if (typeof plugin.initialize === "function") {
        BDfunctionsClays.removeReloadListener(plugin);
        var appwindow = document.querySelector(BDfunctionsClays.dotCN.app);
        if (appwindow) {
            plugin.reloadFix = new MutationObserver((changes, _) => {
                changes.forEach(
                    (change, i) => {
                        if (change.addedNodes) {
                            change.addedNodes.forEach((node) => {
                                if (node && node.classList && node.classList.contains(BDfunctionsClays.disCN.appold)) {
...
BDfunctionsClays.removeReloadListener = function (plugin) {
    if (typeof plugin.initialize === "function" && typeof plugin.reloadFix === "object") {
        plugin.reloadFix.disconnect();
    }
};

BDfunctionsClays.addSettingsButtonListener = function (plugin) {
    if (BDfunctionsClays.isBDv2() && typeof plugin.getSettingsPanel === "function") {
        BDfunctionsClays.removeSettingsButtonListener(plugin);
        BDfunctionsClays.appendSettingsButton(plugin);
...
BDfunctionsClays.appendSettingsButton = function (plugin) {
    let plugincard = document.querySelector(`.bd-card[data-plugin-id=${plugin.id}]`);
    if (plugincard) {
        let settingsbutton = BDfunctionsClays.$(`<div class="DevilBro-settingsbutton bd-button"><span class="bd-material-design-icon"><svg width="18" height="18" viewBox="0 0 24 24"><path d="M12,15.5C10.07,15.5 8.5,13.93 8.5,12C8.5,10.07 10.07,8.5 12,8.5C13.93,8.5 15.5,10.07 15.5,12C15.5,13.93 13.93,15.5 12,15.5M19.43,12.97C19.47,12.65 19.5,12.33 19.5,12C19.5,11.67 19.47,11.34 19.43,11L21.54,9.37C21.73,9.22 21.78,8.95 21.66,8.73L19.66,5.27C19.54,5.05 19.27,4.96 19.05,5.05L16.56,6.05C16.04,5.66 15.5,5.32 14.87,5.07L14.5,2.42C14.46,2.18 14.25,2 14,2H10C9.75,2 9.54,2.18 9.5,2.42L9.13,5.07C8.5,5.32 7.96,5.66 7.44,6.05L4.95,5.05C4.73,4.96 4.46,5.05 4.34,5.27L2.34,8.73C2.21,8.95 2.27,9.22 2.46,9.37L4.57,11C4.53,11.34 4.5,11.67 4.5,12C4.5,12.33 4.53,12.65 4.57,12.97L2.46,14.63C2.27,14.78 2.21,15.05 2.34,15.27L4.34,18.73C4.46,18.95 4.73,19.03 4.95,18.95L7.44,17.94C7.96,18.34 8.5,18.68 9.13,18.93L9.5,21.58C9.54,21.82 9.75,22 10,22H14C14.25,22 14.46,21.82 14.5,21.58L14.87,18.93C15.5,18.67 16.04,18.34 16.56,17.94L19.05,18.95C19.27,19.03 19.54,18.95 19.66,18.73L21.66,15.27C21.78,15.05 21.73,14.78 21.54,14.63L19.43,12.97Z"></path></svg></span></div>`);
        BDfunctionsClays.$(settingsbutton)
            .on("mouseenter.BDfunctionsClaysSettingsButtonListener", (e) => {BDfunctionsClays.createTooltip("Settings", e.currentTarget, {type:"top"});})
            .on("click.BDfunctionsClaysSettingsButtonListener", (e) => {
                var settingsModalMarkup =
                    `<span class="DevilBro-modal DevilBro-settingsmodal ${plugin.id}-settingsmodal">
						<div class="${BDfunctionsClays.disCN.backdrop}"></div>
							<div class="${BDfunctionsClays.disCN.modal}">
								<div class="${BDfunctionsClays.disCN.modalinner}">
									<div class="${BDfunctionsClays.disCNS.modalsub + BDfunctionsClays.disCN.modalsizemedium}">
										<div class="${BDfunctionsClays.disCNS.flex + BDfunctionsClays.disCNS.flex2 + BDfunctionsClays.disCNS.horizontal + BDfunctionsClays.disCNS.horizontal2 + BDfunctionsClays.disCNS.directionrow + BDfunctionsClays.disCNS.justifystart + BDfunctionsClays.disCNS.aligncenter + BDfunctionsClays.disCNS.nowrap + BDfunctionsClays.disCN.modalheader}" style="flex: 0 0 auto;">
											<div class="${BDfunctionsClays.disCN.flexchild}" style="flex: 1 1 auto;">
												<h4 class="${BDfunctionsClays.disCNS.h4 + BDfunctionsClays.disCNS.headertitle + BDfunctionsClays.disCNS.size16 + BDfunctionsClays.disCNS.height20 + BDfunctionsClays.disCNS.weightsemibold + BDfunctionsClays.disCNS.defaultcolor + BDfunctionsClays.disCNS.h4defaultmargin + BDfunctionsClays.disCN.marginreset}">${plugin.name} Settings</h4>
											</div>
											<svg class="${BDfunctionsClays.disCNS.modalclose + BDfunctionsClays.disCN.flexchild}" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 12 12">
									...
									<div class="${BDfunctionsClays.disCNS.scrollerwrap + BDfunctionsClays.disCNS.modalcontent + BDfunctionsClays.disCNS.scrollerthemed + BDfunctionsClays.disCN.themeghosthairline}">
										<div class="${BDfunctionsClays.disCNS.scroller + BDfunctionsClays.disCN.modalsubinner}"></div>
									</div>
								</div>
							</div>
						</div>
					</span>`;
                var settingsModal = BDfunctionsClays.$(settingsModalMarkup);
                settingsModal.find(BDfunctionsClays.dotCN.modalsubinner).append(plugin.getSettingsPanel());
                if (typeof plugin.onSettingsClosed === "function") {
                    settingsModal.on("click.BDfunctionsClaysCloseListener", BDfunctionsClays.dotCNC.modalclose + BDfunctionsClays.dotCN.backdrop, () => {plugin.onSettingsClosed();});
                }
                BDfunctionsClays.appendModal(settingsModal);
            })
            .insertBefore(plugincard.querySelector(".bd-button"));
    }
};

BDfunctionsClays.removeSettingsButtonListener = function (plugin) {
    if (BDfunctionsClays.isBDv2() && typeof plugin.settingsButtonObserver === "object") {
        BDfunctionsClays.$(`.bd-card[data-plugin-id=${plugin.id}] .DevilBro-settingsbutton`).remove();
        plugin.settingsButtonObserver.disconnect();
    }
};

BDfunctionsClays.getLanguageTable = function (lang) {
    var ti = {
    ...
    lang = lang ? lang : BDfunctionsClays.getDiscordLanguage().id;
    return BDfunctionsClays.WebModules.find(function (m) {
        return m.nl === ti[lang];
    });
};

BDfunctionsClays.LanguageStrings = new Proxy(Object.create(null), {
    get: function() {
        var languageStrings = BDfunctionsClays.getLanguageTable();
        if (!languageStrings[arguments[1]]) {
            var englishStrings = BDfunctionsClays.getLanguageTable("en-US");
            if (!englishStrings[arguments[1]]) {
                throw new Error(arguments[1] + " not found in BDfunctionsClays.getLanguageTable");
            }
...
BDfunctionsClays.equals = function (check1, check2, compareOrder) {
    var depth = -1;
...
(() => {
    var pulling = setInterval(() => {
        var UserActions = BDfunctionsClays.WebModules.findByProperties(["getCurrentUser"]);
        var userData = UserActions && typeof UserActions.getCurrentUser == "function" ? UserActions.getCurrentUser() : null;
        if (userData && !BDfunctionsClays.isObjectEmpty(userData)) {
            clearInterval(pulling);
            BDfunctionsClays.myData = userData;
        }
    },100);
})();

BDfunctionsClays.getUserStatus = function (id = BDfunctionsClays.myData.id) {
    id = typeof id == "number" ? id.toFixed() : id;
    var ActivityModule = BDfunctionsClays.WebModules.findByProperties(["getActivity","getStatuses"]);
    var StreamModule = BDfunctionsClays.WebModules.findByProperties(["isStreaming"]);
    return StreamModule.isStreaming(ActivityModule.getActivity(id)) ? "streaming" : ActivityModule.getStatus(id);
};

BDfunctionsClays.getUserAvatar = function (id = BDfunctionsClays.myData.id) {
    id = typeof id == "number" ? id.toFixed() : id;
    var UserStore = BDfunctionsClays.WebModules.findByProperties(["getUser","getUsers"]);
    var IconUtils = BDfunctionsClays.WebModules.findByProperties(["getUserAvatarURL"]);
    var user = UserStore.getUser(id);
    return ((user.avatar ? "" : "https://discordapp.com") + IconUtils.getUserAvatarURL(user)).split("?size")[0];
};

BDfunctionsClays.getChannelAvatar = function (id) {
    id = typeof id == "number" ? id.toFixed() : id;
    var ChannelStore = BDfunctionsClays.WebModules.findByProperties(["getChannel","getChannels"]);
    var IconUtils = BDfunctionsClays.WebModules.findByProperties(["getChannelIconURL"]);
...
BDfunctionsClays.readServerList = function () {
    var server, id, info, foundServers = [], GuildStore = BDfunctionsClays.WebModules.findByProperties(["getGuilds"]);
    for (server of document.querySelectorAll(BDfunctionsClays.dotCN.guildseparator + " ~ " + BDfunctionsClays.dotCN.guild)) {
        id = BDfunctionsClays.getIdOfServer(server);
...
BDfunctionsClays.readUnreadServerList = function (servers) {
    var serverObj, foundServers = [];
    for (serverObj of (servers === undefined || !Array.isArray(servers) ? BDfunctionsClays.readServerList() : servers)) {
        if (serverObj && serverObj.div && (serverObj.div.classList.contains(BDfunctionsClays.disCN.guildunread) || serverObj.div.querySelector(BDfunctionsClays.dotCN.badge))) foundServers.push(serverObj);
    }
    return foundServers;
};

BDfunctionsClays.getSelectedServer = function () {
    var serverObj, id, info;
    id = BDfunctionsClays.WebModules.findByProperties(["getLastSelectedGuildId"]).getGuildId();
    info = id ? BDfunctionsClays.WebModules.findByProperties(["getGuilds"]).getGuild(id) : null;
    if (info) {
        serverObj = BDfunctionsClays.getDivOfServer(id);
        return serverObj ? serverObj : Object.assign({},info,{div:null,data:info});
    }
    else return null;
};

BDfunctionsClays.getIdOfServer = function (server) {
    if (!server || !server.classList || !server.classList.contains(BDfunctionsClays.disCN.guild) || server.classList.contains("copy") || server.classList.contains("folder")) return;
    var switchlink, id;
...
BDfunctionsClays.getDivOfServer = function (id) {
    for (var serverObj of BDfunctionsClays.readServerList()) {
        if (serverObj && serverObj.id == id) return serverObj;
    }
    return null;
};

BDfunctionsClays.readChannelList = function () {
    var channel, info, foundChannels = [], ChannelStore = BDfunctionsClays.WebModules.findByProperties(["getChannels"]);
    for (channel of document.querySelectorAll(BDfunctionsClays.dotCNC.channelcontainerdefault + BDfunctionsClays.dotCN.categorycontainerdefault)) {
        info = BDfunctionsClays.getKeyInformation({"node":channel, "key":"channel"});
        if (info) info = ChannelStore.getChannel(info.id);
        if (info) foundChannels.push(Object.assign({},info,{div:channel,data:info}));
    }
    for (channel of document.querySelectorAll(BDfunctionsClays.dotCN.dmchannel + BDfunctionsClays.dotCN.dmchannelprivate)) {
        info = BDfunctionsClays.getKeyInformation({"node":channel, "key":"user"}) || BDfunctionsClays.getKeyInformation({"node":channel, "key":"channel"});
...
BDfunctionsClays.getSelectedChannel = function () {
    var channelObj, id, info;
    id = BDfunctionsClays.WebModules.findByProperties(["getLastSelectedChannelId"]).getChannelId();
    info = id ? BDfunctionsClays.WebModules.findByProperties(["getChannels"]).getChannel(id) : null;
    if (info) {
        channelObj = BDfunctionsClays.getDivOfChannel(id);
        return channelObj ? channelObj : Object.assign({},info,{div:null,data:info});
    }
    else return null;
};

BDfunctionsClays.getDivOfChannel = function (id) {
    for (var channelObj of BDfunctionsClays.readChannelList()) {
        if (channelObj && channelObj.id == id) return channelObj;
    }
    return null;
};

BDfunctionsClays.readDmList = function () {
    var dm, info, foundDMs = [], ChannelStore = BDfunctionsClays.WebModules.findByProperties(["getChannels"]);
    for (dm of document.querySelectorAll(BDfunctionsClays.dotCN.dms + " > " + BDfunctionsClays.dotCN.guild)) {
        id = BDfunctionsClays.getIdOfDM(dm);
...
BDfunctionsClays.getIdOfDM = function (dm) {
    if (!dm || !dm.classList || !dm.classList.contains(BDfunctionsClays.disCN.guild) || dm.classList.contains("copy") || dm.classList.contains("folder")) return;
    if (!dm.parentElement || !dm.parentElement.classList || !dm.parentElement.classList.contains(BDfunctionsClays.disCN.dms)) return;
    var switchlink, id;
...
BDfunctionsClays.getDivOfDM = function (id) {
    for (var dmObj of BDfunctionsClays.readDmList()) {
        if (dmObj && dmObj.id == id) return dmObj;
    }
    return null;
};

BDfunctionsClays.saveAllData = function (settings, plugin, keyName) {
    if (!BDfunctionsClays.isBDv2()) {
        ...
        let directory = pluginid ? (BDfunctionsClays.Plugins[pluginid] ? BDfunctionsClays.Plugins[pluginid].contentPath : null) : plugin.contentPath;
        if (!directory) return;
        let fs = require("fs");
...
BDfunctionsClays.loadAllData = function (plugin, keyName) {
    if (!BDfunctionsClays.isBDv2()) {
...
BDfunctionsClays.removeAllData = function (plugin, keyName) {
    if (!BDfunctionsClays.isBDv2()) {
        BDfunctionsClays.saveAllData({}, plugin, keyName);
    }
    else {
        let pluginid = typeof plugin === "string" ? plugin.toLowerCase() : null;
        let directory = pluginid ? (BDfunctionsClays.Plugins[pluginid] ? BDfunctionsClays.Plugins[pluginid].contentPath : null) : plugin.contentPath;
...
BDfunctionsClays.getAllData = function (plugin, keyName, compareObject) {
    if (!plugin.defaults || !plugin.defaults[keyName]) return {};
    let oldData = BDfunctionsClays.loadAllData(plugin, keyName), newData = {}, saveData = false;
    ...
    if (saveData) BDfunctionsClays.saveAllData(newData, plugin, keyName);
    return newData;
};

BDfunctionsClays.saveData = function (id, value, plugin, keyName) {
    let data = BDfunctionsClays.loadAllData(plugin, keyName);

    data[id] = value;

    BDfunctionsClays.saveAllData(data, plugin, keyName);
};

BDfunctionsClays.loadData = function (id, plugin, keyName) {
    let data = BDfunctionsClays.loadAllData(plugin, keyName);
...
BDfunctionsClays.removeData = function (id, plugin, keyName) {
    let data = BDfunctionsClays.loadAllData(plugin, keyName);
...
BDfunctionsClays.getData = function (id, plugin, keyName, compareObject) {
    let data = BDfunctionsClays.getAllData(plugin, keyName, compareObject);
...
BDfunctionsClays.appendWebScript = function (filepath) {
    if (!document.head.querySelector("bd-head bd-styles")) BDfunctionsClays.$("head").append(`<bd-head><bd-styles></bd-styles></bd-head>`);
...
BDfunctionsClays.removeWebScript = function (cssname) {
    document.head.querySelectorAll('bd-head bd-styles link[src="' + filepath + '"]').forEach((ele) => {ele.remove();});
};

BDfunctionsClays.appendWebStyle = function (filepath) {
    if (!document.head.querySelector("bd-head bd-styles")) BDfunctionsClays.$("head").append(`<bd-head><bd-styles></bd-styles></bd-head>`);

    BDfunctionsClays.removeWebStyle(filepath);
...
BDfunctionsClays.removeWebStyle = function (cssname) {
    document.head.querySelectorAll('bd-head bd-styles link[href="' + filepath + '"]').forEach((ele) => {ele.remove();});
};

BDfunctionsClays.appendLocalStyle = function (cssname, css) {
    if (!document.head.querySelector("bd-head bd-styles")) BDfunctionsClays.$("head").append(`<bd-head><bd-styles></bd-styles></bd-head>`);

    BDfunctionsClays.removeLocalStyle(cssname);
...
BDfunctionsClays.removeLocalStyle = function (cssname) {
    document.head.querySelectorAll('bd-head bd-styles style[id="' + cssname + 'CSS"]').forEach((ele) => {ele.remove();});
};

BDfunctionsClays.sortArrayByKey = function (array, key, except) {
...
BDfunctionsClays.highlightText = function (string, searchstring) {
    if (searchstring.length < 1) return string;
    var added = 0, copy = string, wrapperopen = `<span class="${BDfunctionsClays.disCN.highlight}">`, wrapperclose = `</span>`;
    BDfunctionsClays.getAllIndexes(string.toUpperCase(), searchstring.toUpperCase()).forEach((start) => {
        let offset = added*(wrapperopen.length + wrapperclose.length);
        start = start + offset;
        let end = start + searchstring.length;
        let openIndexes = [-1].concat(BDfunctionsClays.getAllIndexes(string.substring(0, start), "<"));
        let closedIndexes = [-1].concat(BDfunctionsClays.getAllIndexes(string.substring(0, start), ">"));
...
BDfunctionsClays.getAllIndexes = function (array, val) {
    var indexes = [], i = -1;
...
BDfunctionsClays.formatBytes = function (a, b) {
...
BDfunctionsClays.color2COMP = function (color) {
    if (color) {
        switch (BDfunctionsClays.checkColorType(color)) {
            case "comp":
                return color;
            case "rgb":
...
BDfunctionsClays.color2RGB = function (color) {
    if (color) {
        switch (BDfunctionsClays.checkColorType(color)) {
            case "comp":
            ...
            case "rgb":
                return color;
            case "hsl":
                return BDfunctionsClays.color2RGB(BDfunctionsClays.color2COMP(color));
            case "hex":
                return BDfunctionsClays.color2RGB(BDfunctionsClays.color2COMP(color));
            default:
                return null;
...
BDfunctionsClays.color2HSL = function (color) {
    if (color) {
        switch (BDfunctionsClays.checkColorType(color)) {
            case "comp":
            ...
            case "rgb":
                return BDfunctionsClays.color2HSL(BDfunctionsClays.color2COMP(color));
            case "hsl":
                return color;
            case "hex":
                return BDfunctionsClays.color2HSL(BDfunctionsClays.color2COMP(color));
...
BDfunctionsClays.color2HEX = function (color) {
    if (color) {
        switch (BDfunctionsClays.checkColorType(color)) {
            case "comp":
                return ("#" + (0x1000000 + ((color[2]) | ((color[1]) << 8) | ((color[0]) << 16))).toString(16).slice(1)).toUpperCase();
            case "rgb":
                return BDfunctionsClays.color2HEX(BDfunctionsClays.color2COMP(color));
            case "hsl":
                return BDfunctionsClays.color2HEX(BDfunctionsClays.color2COMP(color));
            case "hex":
                return color;
...
BDfunctionsClays.colorCHANGE = function (color, value) {
    if (color) {
        var comp = BDfunctionsClays.color2COMP(color);
        ...
        switch (BDfunctionsClays.checkColorType(color)) {
            case "comp":
                return comp;
            case "rgb":
                return BDfunctionsClays.color2RGB(comp);
            case "hsl":
                return BDfunctionsClays.color2HSL(comp);
            case "hex":
                return BDfunctionsClays.color2HEX(comp);
            default:
                return null;
        }
    }
...
BDfunctionsClays.colorCOMPARE = function (color1, color2) {
    if (color1 && color2) {
        color1 = BDfunctionsClays.color2RGB(color1);
        color2 = BDfunctionsClays.color2RGB(color2);
        return BDfunctionsClays.equals(color1,color2);
    }
    return null;
};

BDfunctionsClays.colorINV = function (color, conv) {
    if (color) {
        var type = BDfunctionsClays.checkColorType(color);
        if (type) {
            if (conv === undefined) {
                var inv = BDfunctionsClays.color2COMP(color);
                inv = [(255-inv[0]), (255-inv[1]), (255-inv[2])];
                switch (BDfunctionsClays.checkColorType(color)) {
                    case "comp":
                        return inv;
                    case "rgb":
                        return BDfunctionsClays.color2RGB(inv);
                    case "hsl":
                        return BDfunctionsClays.color2HSL(inv);
                    case "hex":
                        return BDfunctionsClays.color2HEX(inv);
                }
            }
            else {
                switch (conv.toLowerCase()) {
                    case "comp":
                        return BDfunctionsClays.colorINV(BDfunctionsClays.color2COMP(color));
                    case "rgb":
                        return BDfunctionsClays.colorINV(BDfunctionsClays.color2RGB(color));
                    case "hsl":
                        return BDfunctionsClays.colorINV(BDfunctionsClays.color2HSL(color));
                    case "hex":
                        return BDfunctionsClays.colorINV(BDfunctionsClays.color2HEX(color));
                    default:
                        return null;
                }
            }
...
BDfunctionsClays.checkColorType = function (color) {
    if (color) {
...
BDfunctionsClays.setInnerText = function (div, text) {
    if (!div) return;
    var textNode = BDfunctionsClays.$(div).contents().filter(function () {return this.nodeType == Node.TEXT_NODE;})[0];
    if (textNode) textNode.textContent = text;
};

BDfunctionsClays.getInnerText = function (div) {
    if (!div) return;
    var textNode = BDfunctionsClays.$(div).contents().filter(function () {return this.nodeType == Node.TEXT_NODE;})[0];
    return textNode ? textNode.textContent : undefined;
};

BDfunctionsClays.encodeToHTML = function (string) {
    var ele = document.createElement("div");
    ele.innerText = string;
    return ele.innerHTML;
};

BDfunctionsClays.regEscape = function (string) {
    return string.replace(/[-/\\^$*+?.()|[\]{}]/g, "\\$&");
};

BDfunctionsClays.insertNRST = function (string) {
    return string
        .replace(new RegExp(BDfunctionsClays.regEscape("\\n"),"g"),"\n")
        .replace(new RegExp(BDfunctionsClays.regEscape("\\r"),"g"),"\r")
        .replace(new RegExp(BDfunctionsClays.regEscape("\\s"),"g")," ")
        .replace(new RegExp(BDfunctionsClays.regEscape("\\t"),"g"),"\t");
};

BDfunctionsClays.clearReadNotifications = function (servers) {
    var GuildActions = BDfunctionsClays.WebModules.findByProperties(["markGuildAsRead"]);
...
BDfunctionsClays.triggerSend = function (textarea) {
    setImmediate(() => {
...
BDfunctionsClays.initElements = function (container) {
    BDfunctionsClays.$(container)
        .off(".BDfunctionsClaysinitElements")
        .on("click.BDfunctionsClaysinitElements", BDfunctionsClays.dotCN.switchinner, (e) => {
            var checked = e.currentTarget.checked;
            BDfunctionsClays.$(e.currentTarget.parentElement)
                .toggleClass(BDfunctionsClays.disCN.switchvaluechecked, checked)
                .toggleClass(BDfunctionsClays.disCN.switchvalueunchecked, !checked);
        })
        .on("click.BDfunctionsClaysinitElements", BDfunctionsClays.dotCNS.checkboxwrapper + BDfunctionsClays.dotCN.checkboxinput, (e) => {
            var checked = e.currentTarget.checked;
            var checkBoxStyle = e.currentTarget.parentElement.querySelector(BDfunctionsClays.dotCN.checkbox);
            BDfunctionsClays.$(checkBoxStyle)
                .toggleClass(BDfunctionsClays.disCN.checkboxchecked, checked)
                .css("background-color", checked ? "rgb(67, 181, 129)" : "")
                .css("border-color", checked ? "rgb(67, 181, 129)" : "")
                .find("polyline")
                .attr("stroke", checked ? "#ffffff" : "transparent");
        })
        .on("click.BDfunctionsClaysinitElements", ".file-navigator", (e) => {
            var filenavigator = e.currentTarget.querySelector("input[type='file']");
            if (filenavigator) filenavigator.click();
        })
        .on("change.BDfunctionsClaysinitElements", "input[type='file']", (e) => {
        ...
        .on("keyup.BDfunctionsClaysinitElements", BDfunctionsClays.dotCNS.searchbar + BDfunctionsClays.dotCN.searchbarinput, (e) => {
            var input = e.currentTarget;
            input.parentElement.querySelector(BDfunctionsClays.dotCN.searchbareyeglass).classList.toggle(BDfunctionsClays.disCN.searchbarvisible, !input.value);
            input.parentElement.querySelector(BDfunctionsClays.dotCN.searchbarclear).classList.toggle(BDfunctionsClays.disCN.searchbarvisible, input.value);
        })
        .on("click.BDfunctionsClaysinitElements", BDfunctionsClays.dotCNS.searchbar + BDfunctionsClays.dotCN.searchbarclear + BDfunctionsClays.dotCN.searchbarvisible, (e) => {
            var clear = e.currentTarget;
            clear.parentElement.parentElement.querySelector(BDfunctionsClays.dotCN.searchbarinput).value = "";
            clear.parentElement.querySelector(BDfunctionsClays.dotCN.searchbareyeglass).classList.add(BDfunctionsClays.disCN.searchbarvisible);
            clear.classList.remove(BDfunctionsClays.disCN.searchbarvisible);
        })
        .on("click.BDfunctionsClaysinitElements", ".numberinput-button-up", (e) => {
        ...
        .on("click.BDfunctionsClaysinitElements", ".tab", (e) => {
            BDfunctionsClays.$(container).find(".tab-content.open").removeClass("open");
            BDfunctionsClays.$(container).find(".tab.selected").removeClass("selected");
            BDfunctionsClays.$(container).find(".tab-content[tab='" + BDfunctionsClays.$(e.currentTarget).attr("tab") + "']").addClass("open");
            BDfunctionsClays.$(e.currentTarget).addClass("selected");
        });

    BDfunctionsClays.$(container).find(".tab").first().addClass("selected");
    BDfunctionsClays.$(container).find(".tab-content").first().addClass("open");

    var libraryStrings = BDfunctionsClays.getLibraryStrings();
    BDfunctionsClays.$(container).find(".btn-save " + BDfunctionsClays.dotCN.buttoncontents).text(libraryStrings.btn_save_text);
    BDfunctionsClays.$(container).find(".btn-cancel " + BDfunctionsClays.dotCN.buttoncontents).text(libraryStrings.btn_cancel_text);
    BDfunctionsClays.$(container).find(".btn-all " + BDfunctionsClays.dotCN.buttoncontents).text(libraryStrings.btn_all_text);
    BDfunctionsClays.$(container).find(".btn-add " + BDfunctionsClays.dotCN.buttoncontents).text(libraryStrings.btn_add_text);
    BDfunctionsClays.$(container).find(".btn-ok " + BDfunctionsClays.dotCN.buttoncontents).text(libraryStrings.btn_ok_text);
    BDfunctionsClays.$(container).find(".file-navigator " + BDfunctionsClays.dotCN.buttoncontents).text(libraryStrings.file_navigator_text);
    BDfunctionsClays.$(container).find(BDfunctionsClays.dotCNS.searchbar + BDfunctionsClays.dotCN.searchbarinput).attr("placeholder", libraryStrings.search_placeholder);

    BDfunctionsClays.$(container)
        .find(BDfunctionsClays.dotCN.switchinner).each((_, switchinner) => {
        BDfunctionsClays.$(switchinner.parentElement)
            .toggleClass(BDfunctionsClays.disCN.switchvaluechecked, switchinner.checked)
            .toggleClass(BDfunctionsClays.disCN.switchvalueunchecked, !switchinner.checked);
    });

    BDfunctionsClays.$(container)
        .find(BDfunctionsClays.dotCNS.checkboxwrapper + BDfunctionsClays.dotCN.checkboxinput).each((_, checkBox) => {
        if (checkBox.checked) {
            BDfunctionsClays.$(checkBox.parentElement.querySelector(BDfunctionsClays.dotCN.checkbox))
                .addClass(BDfunctionsClays.disCN.checkboxchecked)
...
BDfunctionsClays.appendModal = function (modal) {
    let id = Math.round(Math.random()*10000000000000000);
    var container = document.querySelector(BDfunctionsClays.dotCN.app + " ~ [class^='theme']:not([class*='popouts'])");
    if (!container) return;

    BDfunctionsClays.$(modal)
        .appendTo(container)
        .on("click", BDfunctionsClays.dotCNC.backdrop + BDfunctionsClays.dotCNC.modalclose + ".btn-save, .btn-send, .btn-cancel, .btn-ok", () => {
            BDfunctionsClays.$(document).off("keydown.BDfunctionsClaysmodalEscapeListener" + id);
            BDfunctionsClays.$(modal).addClass("closing");
            setTimeout(() => {modal.remove();}, 300);
        });


    BDfunctionsClays.initElements(modal);

    BDfunctionsClays.$(document)
        .off("keydown.BDfunctionsClaysmodalEscapeListener" + id)
        .on("keydown.BDfunctionsClaysmodalEscapeListener" + id, (e) => {
            if (e.which == 27) BDfunctionsClays.$(modal).find(BDfunctionsClays.dotCN.backdrop).click();
        });
};

BDfunctionsClays.updateContextPosition = function (context) {
    var app = document.querySelector(BDfunctionsClays.dotCN.appmount);
    var menuWidth = BDfunctionsClays.$(context).outerWidth();
    var menuHeight = BDfunctionsClays.$(context).outerHeight();
    var position = BDfunctionsClays.mousePosition;
    var newposition = {
        x: position.x - menuWidth,
        y: position.y - menuHeight
    };
    BDfunctionsClays.$(context)
...
BDfunctionsClays.appendContextMenu = function (context, e) {
    BDfunctionsClays.$(BDfunctionsClays.dotCN.tooltips).before(context);
    ...
    BDfunctionsClays.$(context)
        ...
        .toggleClass(BDfunctionsClays.disCN.contextmenuinvertchildx, e.pageX + menusizes.width > document.body.firstElementChild.offsetWidth)
        .toggleClass("invertY", e.pageY + menusizes.height > document.body.firstElementChild.offsetHeight)
        .addClass(BDfunctionsClays.getDiscordTheme());

    BDfunctionsClays.updateContextPosition(context);

    BDfunctionsClays.$(document).on("mousedown.BDfunctionsClaysContextMenu", (e2) => {
        if (BDfunctionsClays.$(context).has(e2.target).length == 0 && context != e2.target) {
            BDfunctionsClays.$(document).off("mousedown.BDfunctionsClaysContextMenu");
            context.remove();
        }
        else {
            var item = BDfunctionsClays.$(BDfunctionsClays.dotCN.contextmenuitem).has(e2.target)[0];
            if (item && !item.classList.contains(BDfunctionsClays.disCN.contextmenuitemdisabled) && !item.classList.contains(BDfunctionsClays.disCN.contextmenusubmenu) && !item.classList.contains(BDfunctionsClays.disCN.contextmenuitemtoggle)) {
                BDfunctionsClays.$(document).off("mousedown.BDfunctionsClaysContextMenu");
            }
        }
    });
...
BDfunctionsClays.appendSubMenu = function (target, menu) {
    BDfunctionsClays.$(target).append(menu);
    var offsets = BDfunctionsClays.$(target).offset();
    var menuHeight = BDfunctionsClays.$(menu).outerHeight();
    BDfunctionsClays.$(menu)
        .addClass(BDfunctionsClays.getDiscordTheme())
        .css("left", offsets.left + "px")
        .css("top", offsets.top + menuHeight > window.outerHeight ? (offsets.top - menuHeight + BDfunctionsClays.$(target).outerHeight()) + "px" : offsets.top + "px");

    BDfunctionsClays.$(target).on("mouseleave.BDfunctionsClaysSubContextMenu", () => {
        BDfunctionsClays.$(target).off("mouseleave.BDfunctionsClaysSubContextMenu");
        menu.remove();
    });
};

BDfunctionsClays.setColorSwatches = function (currentCOMP, wrapper, swatch) {
    var wrapperDiv = BDfunctionsClays.$(wrapper);
    ...
    var swatches =
        `<div class="${BDfunctionsClays.disCNS.flex + BDfunctionsClays.disCNS.flex2 + BDfunctionsClays.disCNS.horizontal + BDfunctionsClays.disCNS.horizontal2 + BDfunctionsClays.disCNS.directionrow + BDfunctionsClays.disCNS.justifystart + BDfunctionsClays.disCNS.alignstretch + BDfunctionsClays.disCN.nowrap}" style="flex: 1 1 auto; margin-top: 5px;">
			...
			<div class="${BDfunctionsClays.disCNS.flex + BDfunctionsClays.disCNS.flex2 + BDfunctionsClays.disCNS.horizontal + BDfunctionsClays.disCNS.horizontal2 + BDfunctionsClays.disCNS.directionrow + BDfunctionsClays.disCNS.justifystart + BDfunctionsClays.disCNS.alignstretch + BDfunctionsClays.disCN.wrap} ui-color-picker-row" style="flex: 1 1 auto; display: flex; flex-wrap: wrap; overflow: visible !important;">
    ...
    BDfunctionsClays.$(swatches).appendTo(wrapperDiv);

    if (currentCOMP) {
        var currentRGB = BDfunctionsClays.color2RGB(currentCOMP);
        var invRGB = BDfunctionsClays.colorINV(currentRGB);
        ...
        if (selection > -1) {
            ...
            BDfunctionsClays.$(".custom", wrapperDiv)
                .addClass("selected")
                .css("background-color", currentRGB)
                .css("border", "4px solid " + invRGB);

            BDfunctionsClays.$(".color-picker-dropper-fg", wrapperDiv)
                .attr("fill", currentCOMP[0] > 150 && currentCOMP[1] > 150 && currentCOMP[2] > 150 ? "#000000" : "#ffffff");
        }
    }
    else {
        BDfunctionsClays.$(".nocolor", wrapperDiv)
            .addClass("selected")
    ...
    wrapperDiv.on("click", ".ui-color-picker-" + swatch + ":not(.custom)", (e) => {
        ...
        var bgColor = BDfunctionsClays.$(e.target).css("background-color");
        var newInvRGB = BDfunctionsClays.checkColorType(bgColor) ? BDfunctionsClays.colorINV(bgColor,"rgb") : "black";
        ...
        BDfunctionsClays.$(e.currentTarget)
            .addClass("selected")
    ...
    wrapperDiv.on("click", ".ui-color-picker-" + swatch + ".custom", (e) => {
        if (wrapperDiv.hasClass("disabled")) return;
        BDfunctionsClays.openColorPicker(e.currentTarget.style.backgroundColor, swatch);
    });
};

BDfunctionsClays.openColorPicker = function (currentColor, swatch) {
    var libraryStrings = BDfunctionsClays.getLibraryStrings();
    var inputs = {
			...
			<div class="${BDfunctionsClays.disCN.modal}">
				<div class="${BDfunctionsClays.disCN.modalinner}">
					<div class="${BDfunctionsClays.disCNS.modalsub + BDfunctionsClays.disCN.modalsizemedium}">
						<div class="${BDfunctionsClays.disCNS.flex + BDfunctionsClays.disCNS.flex2 + BDfunctionsClays.disCNS.horizontal + BDfunctionsClays.disCNS.horizontal2 + BDfunctionsClays.disCNS.directionrow + BDfunctionsClays.disCNS.justifystart + BDfunctionsClays.disCNS.aligncenter + BDfunctionsClays.disCNS.nowrap + BDfunctionsClays.disCN.modalheader}" style="flex: 0 0 auto;">
							<div class="${BDfunctionsClays.disCN.flexchild}" style="flex: 1 1 auto;">
								<h4 class="${BDfunctionsClays.disCNS.h4 + BDfunctionsClays.disCNS.headertitle + BDfunctionsClays.disCNS.size16 + BDfunctionsClays.disCNS.height20 + BDfunctionsClays.disCNS.weightsemibold + BDfunctionsClays.disCNS.defaultcolor + BDfunctionsClays.disCNS.h4defaultmargin + BDfunctionsClays.disCN.marginreset}">${libraryStrings.colorpicker_modal_header_text}</h4>
							</div>
							<svg class="${BDfunctionsClays.disCNS.modalclose + BDfunctionsClays.disCN.flexchild}" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 12 12">
								<g fill="none" fill-rule="evenodd">
									<path d="M0 0h12v12H0"></path>
						...
						<div class="${BDfunctionsClays.disCNS.flex + BDfunctionsClays.disCNS.flex2 + BDfunctionsClays.disCNS.vertical + BDfunctionsClays.disCNS.modalsubinner + BDfunctionsClays.disCNS.directioncolumn + BDfunctionsClays.disCNS.justifystart + BDfunctionsClays.disCNS.alignstretch + BDfunctionsClays.disCN.nowrap} colorpicker-container" style="flex: 1 1 auto;">
							...
							<div class="colorpicker-controls">
								...
								<div class="colorpicker-inputs ${BDfunctionsClays.disCNS.card + BDfunctionsClays.disCN.cardprimaryeditable}">
									${Object.keys(inputs).map((key, i) =>
            `<div class="colorpicker-input ${BDfunctionsClays.disCNS.flex + BDfunctionsClays.disCNS.flex2 + BDfunctionsClays.disCNS.horizontal + BDfunctionsClays.disCNS.horizontal2 + BDfunctionsClays.disCNS.directionrow + BDfunctionsClays.disCNS.aligncenter + BDfunctionsClays.disCNS.justifycenter + BDfunctionsClays.disCNS.margintop4 + BDfunctionsClays.disCNS.marginbottom4 + BDfunctionsClays.disCN.nowrap}">
										<div class="${BDfunctionsClays.disCNS.flex + BDfunctionsClays.disCNS.flex2 + BDfunctionsClays.disCNS.horizontal + BDfunctionsClays.disCNS.horizontal2 + BDfunctionsClays.disCNS.directionrow + BDfunctionsClays.disCN.nowrap}" style="flex: 1 1 20%">
											<h5 class="${BDfunctionsClays.disCNS.h5 + BDfunctionsClays.disCNS.size12 + BDfunctionsClays.disCNS.height16 + BDfunctionsClays.disCN.weightsemibold}">${key}:</h5>
										</div>
										<div class="${inputs[key].type == 'number' ? 'inputNumberWrapper inputNumberWrapperMini ' : ''}${BDfunctionsClays.disCNS.inputwrapper + BDfunctionsClays.disCNS.vertical + BDfunctionsClays.disCNS.flex + BDfunctionsClays.disCN.directioncolumn}" style="flex: 1 1 80%;">
											...
											<input type="${inputs[key].type}"${!isNaN(inputs[key].min) && inputs[key].min != null ? ' min="' + inputs[key].min + '"' : ''}${!isNaN(inputs[key].max) && inputs[key].max != null ? ' max="' + inputs[key].max + '"' : ''}${!isNaN(inputs[key].length) && inputs[key].length != null ? ' maxlength="' + inputs[key].length + '"' : ''} name="${inputs[key].group}" placeholder="${inputs[key].default}" class="${BDfunctionsClays.disCNS.inputmini + BDfunctionsClays.disCNS.input + BDfunctionsClays.disCN.size16} colorpicker-${inputs[key].name}">
										</div>
									</div>`).join("")}
								</div>
							</div>
						</div>
						<div class="${BDfunctionsClays.disCNS.flex + BDfunctionsClays.disCNS.flex2 + BDfunctionsClays.disCNS.horizontalreverse + BDfunctionsClays.disCNS.horizontalreverse2 + BDfunctionsClays.disCNS.directionrowreverse + BDfunctionsClays.disCNS.justifystart + BDfunctionsClays.disCNS.alignstretch + BDfunctionsClays.disCNS.nowrap + BDfunctionsClays.disCN.modalfooter}">
							<button type="button" class="btn-ok ${BDfunctionsClays.disCNS.button + BDfunctionsClays.disCNS.buttonlookfilled + BDfunctionsClays.disCNS.buttoncolorbrand + BDfunctionsClays.disCNS.buttonsizemedium + BDfunctionsClays.disCN.buttongrow}">
								<div class="${BDfunctionsClays.disCN.buttoncontents}"></div>
							</button>
						</div>
					</div>
				</div>
    ...
    var colorPickerModal = BDfunctionsClays.$(colorPickerModalMarkup)[0];
    BDfunctionsClays.appendModal(colorPickerModal);
    BDfunctionsClays.$(colorPickerModal)
        .on("click", ".btn-ok", () => {
            var newRGB = colorPickerModal.querySelector("[class^='colorpicker-preview-'].selected").style.backgroundColor;
            var newCOMP = BDfunctionsClays.color2COMP(newRGB);
            var newInvRGB = BDfunctionsClays.colorINV(newRGB);

            BDfunctionsClays.$(".ui-color-picker-" + swatch + ".selected.nocolor")
                .removeClass("selected")
                .css("border", "4px solid red");

            BDfunctionsClays.$(".ui-color-picker-" + swatch + ".selected")
                .removeClass("selected")
                .css("border", "4px solid transparent");

            BDfunctionsClays.$(".ui-color-picker-" + swatch + ".custom")
                .addClass("selected")
                .css("background-color", newRGB)
                .css("border", "4px solid " + newInvRGB);

            BDfunctionsClays.$(".color-picker-dropper-fg-" + swatch)
    ...
    var pMinX = BDfunctionsClays.$(ppane).offset().left;
    var pMaxX = pMinX + ppane.offsetWidth;
    var pMinY = BDfunctionsClays.$(ppane).offset().top;
    ...
    var sMinY = BDfunctionsClays.$(spane).offset().top;
    var sMaxY = sMinY + spane.offsetHeight;

    [hue, saturation, lightness] = BDfunctionsClays.color2HSL(currentColor).replace(new RegExp(" ", "g"), "").slice(4, -1).split(",");
    saturation *= 100;
    lightness *= 100;
    updateAllValues();
    updateCursors();

    BDfunctionsClays.$(ppane)
        .on("mousedown", (e) => {
            BDfunctionsClays.appendLocalStyle("crossHairColorPicker", `* {cursor: crosshair !important;}`);

            switchPreviews(e.button);

            pHalfW = pcursor.offsetWidth/2;
            pHalfH = pcursor.offsetHeight/2;
            pMinX = BDfunctionsClays.$(ppane).offset().left;
            pMaxX = pMinX + ppane.offsetWidth;
            pMinY = BDfunctionsClays.$(ppane).offset().top;
            pMaxY = pMinY + ppane.offsetHeight;
            pX = e.clientX - pHalfW;
            pY = e.clientY - pHalfH;

            BDfunctionsClays.$(pcursor).offset({"left":pX,"top":pY});

            saturation = BDfunctionsClays.mapRange([pMinX - pHalfW, pMaxX - pHalfW], [0, 100], pX);
            lightness = BDfunctionsClays.mapRange([pMinY - pHalfH, pMaxY - pHalfH], [100, 0], pY);
            updateAllValues();

            BDfunctionsClays.$(document)
                .off("mouseup.ColorPicker").off("mousemove.ColorPicker")
                .on("mouseup.ColorPicker", () => {
                    BDfunctionsClays.removeLocalStyle("crossHairColorPicker");
                    BDfunctionsClays.$(document).off("mouseup.ColorPicker").off("mousemove.ColorPicker");
                })
    ...
    BDfunctionsClays.$(spane)
        .on("mousedown", (e) => {
            ...
            sMinY = BDfunctionsClays.$(spane).offset().top;
            sMaxY = sMinY + spane.offsetHeight;
            sY = e.clientY - sHalfH;

            BDfunctionsClays.$(scursor).offset({"top":sY});

            hue = BDfunctionsClays.mapRange([sMinY - sHalfH, sMaxY - sHalfH], [360, 0], sY);
            updateAllValues();
    ...
    BDfunctionsClays.$(colorPickerModal)
        .on("input", BDfunctionsClays.dotCN.inputmini, (e) => {
    ...
    BDfunctionsClays.$(colorPickerModal)
        .on("click", "[class^='colorpicker-preview-']", (e) => {
            ...
            [hue, saturation, lightness] = BDfunctionsClays.color2HSL(e.currentTarget.style.backgroundColor).replace(new RegExp(" ", "g"), "").slice(4, -1).split(",");
            saturation *= 100;
            lightness *= 100;
            updateAllValues();
            updateCursors();
    ...
    function updateValues (type) {
        switch (type) {
            case "hex":
                ...
                if (/^#([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})BDfunctionsClays.$/i.test(hex)) {
                    [red, green, blue] = BDfunctionsClays.color2COMP(hex);
                    [hue, saturation, lightness] = BDfunctionsClays.color2HSL(hex).replace(new RegExp(" ", "g"), "").slice(4, -1).split(",");
                    saturation *= 100;
                    lightness *= 100;
            ...
            case "rgb":
                ...
                if (red && red >= 0 && red <= 255 && green && green >= 0 && green <= 255 && blue && blue >= 0 && blue <= 255) {
                    [hue, saturation, lightness] = BDfunctionsClays.color2HSL([red, green, blue]).replace(new RegExp(" ", "g"), "").slice(4, -1).split(",");
                    saturation *= 100;
                    lightness *= 100;
                    colorPickerModal.querySelector(".colorpicker-hex").value = BDfunctionsClays.color2HEX([red, green, blue]);
                    colorPickerModal.querySelector(".colorpicker-hue").value = Math.round(hue);
                    colorPickerModal.querySelector(".colorpicker-saturation").value = Math.round(saturation);
                    colorPickerModal.querySelector(".colorpicker-lightness").value = Math.round(lightness);
                }
            ...
            case "hsl":
                ...
                if (hue && hue >= 0 && hue <= 360 && saturation && saturation >= 0 && saturation <= 100 && lightness && lightness >= 0 && lightness <= 100) {
                    [red, green, blue] = BDfunctionsClays.color2COMP("hsl(" + hue + ", " + saturation/100 + ", " + lightness/100 + ")");
                    colorPickerModal.querySelector(".colorpicker-hex").value = BDfunctionsClays.color2HEX([red, green, blue]);
                    colorPickerModal.querySelector(".colorpicker-red").value = red;
                    colorPickerModal.querySelector(".colorpicker-green").value = green;
                    colorPickerModal.querySelector(".colorpicker-blue").value = blue;
    ...
    function updateCursors () {
        sHalfH = scursor.offsetHeight/2;
        sMinY = BDfunctionsClays.$(spane).offset().top;
        sY = BDfunctionsClays.mapRange([360, 0], [sMinY - sHalfH, sMaxY - sHalfH], hue);
        ...
        pMinY = BDfunctionsClays.$(ppane).offset().top;
        pMaxY = pMinY + ppane.offsetHeight;
        pX = BDfunctionsClays.mapRange([0, 100], [pMinX - pHalfW, pMaxX - pHalfW], saturation);
        pY = BDfunctionsClays.mapRange([100, 0], [pMinY - pHalfH, pMaxY - pHalfH], lightness);

        BDfunctionsClays.$(scursor).offset({"top":sY});
        BDfunctionsClays.$(pcursor).offset({"left":pX,"top":pY});
        BDfunctionsClays.$(pcursor).find("circle").attr("stroke", BDfunctionsClays.colorINV([red, green, blue], "rgb"));
        BDfunctionsClays.$(scursor).find("path").attr("stroke", BDfunctionsClays.color2RGB("hsl(" + hue + ", 1, 1)"));
    }
    ...
    function updateColors () {
        colorPickerModal.querySelector(".colorpicker-color").style.background = BDfunctionsClays.color2RGB("hsl(" + hue + ", 1, 1)");
        colorPickerModal.querySelector("[class^='colorpicker-preview-'].selected").style.background = BDfunctionsClays.color2RGB([red, green, blue]);
        colorPickerModal.querySelector("[class^='colorpicker-preview-'].selected").style.borderColor = BDfunctionsClays.colorINV([red, green, blue], "rgb");
    }
};

BDfunctionsClays.mapRange = function (from, to, number) {
    return to[0] + (number - from[0]) * (to[1] - to[0]) / (from[1] - from[0]);
};

BDfunctionsClays.getSwatchColor = function (swatch) {
    return !BDfunctionsClays.$(".ui-color-picker-" + swatch + ".nocolor.selected")[0] ? BDfunctionsClays.color2COMP(BDfunctionsClays.$(".ui-color-picker-" + swatch + ".selected").css("background-color")) : null;
};

BDfunctionsClays.zacksFork = function () {
    return (typeof bdpluginErrors === "object" && typeof bdthemeErrors === "object" && typeof bbdVersion === "string");
};

BDfunctionsClays.isBDv2 = function () {
    return (typeof BDfunctionsClays.BDv2Api !== "undefined");
};

BDfunctionsClays.getLibraryStrings = function () {
    switch (BDfunctionsClays.getDiscordLanguage().id) {
        case "hr": 		//croatian
            return {
...
BDfunctionsClays.$(document)
    .off("click.BDfunctionsClaysPluginClick")
    .off("keydown.BDfunctionsClaysPressedKeys")
    .off("keyup.BDfunctionsClaysPressedKeys")
    .off("mousedown.BDfunctionsClaysMousePosition")
    .on("click.BDfunctionsClaysPluginClick", ".bd-settingswrap .bd-refresh-button, .bd-settingswrap .bd-switch-checkbox", () => {
        BDfunctionsClays.setPluginCache();
        BDfunctionsClays.setThemeCache();
    })
    .on("keydown.BDfunctionsClaysPressedKeys", (e) => {
        if (!BDfunctionsClays.pressedKeys.includes(e.which)) BDfunctionsClays.pressedKeys.push(e.which);
    })
    .on("keyup.BDfunctionsClaysPressedKeys", (e) => {
        BDfunctionsClays.removeFromArray(BDfunctionsClays.pressedKeys, e.which);
    })
    .on("mousedown.BDfunctionsClaysMousePosition", (e) => {
        BDfunctionsClays.mousePosition = {x:e.pageX,y:e.pageY};
    });

BDfunctionsClays.isPluginEnabled = function (name) {
    if (!BDfunctionsClays.isBDv2()) return window.bdplugins[name] && window.pluginCookie[name];
    else return BDfunctionsClays.Plugins[name.toLowerCase()] ? BDfunctionsClays.Plugins[name.toLowerCase()].enabled : null;
};

BDfunctionsClays.isRestartNoMoreEnabled = function () {
    return BDfunctionsClays.isPluginEnabled("Restart-No-More") || BDfunctionsClays.isPluginEnabled("Restart No More");
};

BDfunctionsClays.isThemeEnabled = function (name) {
    if (!BDfunctionsClays.isBDv2()) return window.bdthemes[name] && window.themeCookie[name];
    else return BDfunctionsClays.Themes[name.toLowerCase()] ? BDfunctionsClays.Themes[name.toLowerCase()].enabled : null;
};

(BDfunctionsClays.setPluginCache = function () {
    if (!BDfunctionsClays.isBDv2()) return;
    BDfunctionsClays.Plugins = {};
    for (let id of BDfunctionsClays.BDv2Api.Plugins.listPlugins()) {
        BDfunctionsClays.BDv2Api.Plugins.getPlugin(id).then(plugin => {BDfunctionsClays.Plugins[id] = plugin;});
    }
})();

(BDfunctionsClays.setThemeCache = function () {
    if (!BDfunctionsClays.isBDv2()) return;
    BDfunctionsClays.Themes = {};
    for (let id of BDfunctionsClays.BDv2Api.Themes.listThemes()) {
        BDfunctionsClays.BDv2Api.Themes.getTheme(id).then(theme => {BDfunctionsClays.Themes[id] = theme;});
    }
})();

BDfunctionsClays.DiscordClasses = {
...
// stolen from square :-*
BDfunctionsClays.disCN = new Proxy(Object.create(null), {
    get: function() {
        if (BDfunctionsClays.DiscordClasses[arguments[1]] === undefined) {
            throw new Error(arguments[1] + " not found in BDfunctionsClays.DiscordClasses");
        }
        return BDfunctionsClays.DiscordClasses[arguments[1]];
    }
});

BDfunctionsClays.disCNS = new Proxy(Object.create(null), {
    get: function() {
        if (BDfunctionsClays.DiscordClasses[arguments[1]] === undefined) {
            throw new Error(arguments[1] + " not found in BDfunctionsClays.DiscordClasses");
        }
...
BDfunctionsClays.disCNC = new Proxy(Object.create(null), {
    get: function() {
        if (BDfunctionsClays.DiscordClasses[arguments[1]] === undefined) {
            throw new Error(arguments[1] + " not found in BDfunctionsClays.DiscordClasses");
        }
...
BDfunctionsClays.dotCN = new Proxy(Object.create(null), {
    get: function() {
        if (BDfunctionsClays.DiscordClasses[arguments[1]] === undefined) {
            throw new Error(arguments[1] + " not found in BDfunctionsClays.DiscordClasses");
        }
...
BDfunctionsClays.dotCNS = new Proxy(Object.create(null), {
    get: function() {
        if (BDfunctionsClays.DiscordClasses[arguments[1]] === undefined) {
            throw new Error(arguments[1] + " not found in BDfunctionsClays.DiscordClasses");
        }
...
BDfunctionsClays.dotCNC = new Proxy(Object.create(null), {
    get: function() {
        if (BDfunctionsClays.DiscordClasses[arguments[1]] === undefined) {
            throw new Error(arguments[1] + " not found in BDfunctionsClays.DiscordClasses");
        }
...
BDfunctionsClays.idCN = new Proxy(Object.create(null), {
    get: function() {
        if (BDfunctionsClays.DiscordClasses[arguments[1]] === undefined) {
            throw new Error(arguments[1] + " not found in BDfunctionsClays.DiscordClasses");
        }
...
BDfunctionsClays.idCNS = new Proxy(Object.create(null), {
    get: function() {
        if (BDfunctionsClays.DiscordClasses[arguments[1]] === undefined) {
            throw new Error(arguments[1] + " not found in BDfunctionsClays.DiscordClasses");
        }
...
BDfunctionsClays.idCNC = new Proxy(Object.create(null), {
    get: function() {
        if (BDfunctionsClays.DiscordClasses[arguments[1]] === undefined) {
            throw new Error(arguments[1] + " not found in BDfunctionsClays.DiscordClasses");
        }
...
BDfunctionsClays.appendLocalStyle("BDfunctionsClays", `
	...
	.DevilBro-settings ${BDfunctionsClays.dotCN.margintop4} {
		margin-top: 4px !important;
	}
	.DevilBro-settings ${BDfunctionsClays.dotCN.margintop8} {
		margin-top: 8px !important;
	}
	.DevilBro-settings ${BDfunctionsClays.dotCN.margintop20}{
		margin-top: 20px !important;
	}
	.DevilBro-settings ${BDfunctionsClays.dotCN.margintop40} {
		margin-top: 40px !important;
	}
	.DevilBro-settings ${BDfunctionsClays.dotCN.margintop60} {
	...
	.DevilBro-modal .inputNumberWrapper .numberinput-buttons-zone:hover + ${BDfunctionsClays.dotCN.input},
	.DevilBro-settings .inputNumberWrapper .numberinput-buttons-zone:hover + ${BDfunctionsClays.dotCN.input} {
		border-color: black;
	}
	.DevilBro-modal .inputNumberWrapper .numberinput-buttons-zone:hover + ${BDfunctionsClays.dotCN.input}:focus,
	.DevilBro-settings .inputNumberWrapper .numberinput-buttons-zone:hover + ${BDfunctionsClays.dotCN.input}:focus,
	.DevilBro-modal .inputNumberWrapper .numberinput-buttons-zone.pressed + ${BDfunctionsClays.dotCN.input},
	.DevilBro-settings .inputNumberWrapper .numberinput-buttons-zone.pressed + ${BDfunctionsClays.dotCN.input} {
		border-color: #7289da;
	}
	...
	.DevilBro-modal .inputNumberWrapper ${BDfunctionsClays.dotCN.input}[type=number],
	.DevilBro-settings .inputNumberWrapper ${BDfunctionsClays.dotCN.input}[type=number] {
		padding-right: 25px;
	}
	.DevilBro-modal .inputNumberWrapper ${BDfunctionsClays.dotCN.input}[type=number]::-webkit-inner-spin-button,
	.DevilBro-modal .inputNumberWrapper ${BDfunctionsClays.dotCN.input}[type=number]::-webkit-outer-spin-button,
	.DevilBro-settings .inputNumberWrapper ${BDfunctionsClays.dotCN.input}[type=number]::-webkit-inner-spin-button,
	.DevilBro-settings .inputNumberWrapper ${BDfunctionsClays.dotCN.input}[type=number]::-webkit-outer-spin-button {
	...
	.DevilBro-settings ${BDfunctionsClays.dotCN.hovercard},
	.DevilBro-settings ${BDfunctionsClays.dotCNS.hovercard + BDfunctionsClays.dotCN.hovercardinner} {
		width: 550px;
		min-height: 28px;
	}
	...
	.DevilBro-settingsmodal .DevilBro-settings ${BDfunctionsClays.dotCN.hovercard},
	.DevilBro-settingsmodal .DevilBro-settings ${BDfunctionsClays.dotCNS.hovercard + BDfunctionsClays.dotCN.hovercardinner} {
		width: 520px;
	}

	.DevilBro-settings ${BDfunctionsClays.dotCN.hovercard}:before {
		z-index: 50;
		left: -10px;
	}
	...
	.DevilBro-settings ${BDfunctionsClays.dotCNS.hovercard + BDfunctionsClays.dotCNS.hovercardbutton} {
		opacity: 0;
		position: absolute;
		right: -31px;
		top: -12px;
	...
	.DevilBro-settings ${BDfunctionsClays.dotCN.hovercard}:hover ${BDfunctionsClays.dotCNS.hovercard} {
		opacity: 1;
	}

	.DevilBro-modal ${BDfunctionsClays.dotCN.checkboxcontainer},
	.DevilBro-settings ${BDfunctionsClays.dotCN.checkboxcontainer} {
		margin-left: 10px;
	}

	.DevilBro-modal ${BDfunctionsClays.dotCN.checkboxcontainer}:before,
	.DevilBro-settings ${BDfunctionsClays.dotCN.checkboxcontainer}:before {
		display: none;
	}
	...
	.DevilBro-modal ${BDfunctionsClays.dotCN.backdrop} {
	...
	.DevilBro-modal.closing ${BDfunctionsClays.dotCN.backdrop} {
	...
	.DevilBro-modal ${BDfunctionsClays.dotCN.modal} {
		animation: animation-modal 250ms cubic-bezier(0.175, 0.885, 0.32, 1.275);
		animation-fill-mode: forwards;
		transform: scale(0.7);
		transform-origin: 50% 50%;
	}
	.DevilBro-modal.closing ${BDfunctionsClays.dotCN.modal} {
	...
	.DevilBro-settingsmodal ${BDfunctionsClays.dotCN.modal} {
		z-index: 4010;
	}

	.DevilBro-settingsmodal ${BDfunctionsClays.dotCN.backdrop} {
		z-index: 4005;
	}
	...
	.colorpicker-modal ${BDfunctionsClays.dotCN.modalsub} {
		width: 600px;
	}
